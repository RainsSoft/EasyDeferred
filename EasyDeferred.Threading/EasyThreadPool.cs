...
namespace EasyDeferred.EasyThreadPool
{
    ...
    public class EasyThreadPool : IDisposable
    {
        ...
        private Dictionary<string, EasyTask> publicPool; //所有的线程

        public Dictionary<string, EasyTask> PublicPool {
            get { return publicPool; }
            //set { publicPool = value; }
        }

        private Queue<EasyTask> freeQueue;  //空闲线程队列

        private Dictionary<string, EasyTask> working;   //正在工作的线程

        private List<string> workingKeys;

        private Queue<EasyWaitItem> waitQueue;  //等待执行工作队列

        private static EasyThreadPool threadPoolManager = null;
        ...
        private EasyThreadPool(int min, int max, int increment) {
            this.min = min;
            this.max = max;
        ...
        /// <returns></returns>
        public static EasyThreadPool getInstance(int min, int max, int increment) {
            if (threadPoolManager == null) {
                threadPoolManager = new EasyThreadPool(min, max, increment);
            }
        ...
        public static EasyThreadPool getInstance() {
            if (threadPoolManager == null) {
                threadPoolManager = new EasyThreadPool(1, 5, 1);
            }
            return threadPoolManager;
        }
        ...
        void initThreadTool() {
            ...
            publicPool = new Dictionary<string, EasyTask>();

            working = new Dictionary<string, EasyTask>();

            workingKeys = new List<string>();

            freeQueue = new Queue<EasyTask>();

            waitQueue = new Queue<EasyWaitItem>();
            ...
            for (int i = 0; i < min; i++) {
                ...
                t.WorkComplete += new Action<EasyTask>(t_WorkComplete);
                //加入到所有线程的字典中
        ...
        //线程执行完毕后的触发事件
        void t_WorkComplete(EasyTask obj) {
            obj.WorkComplete -= new Action<EasyTask>(t_WorkComplete);
            lock (lockerObj) {
                ...
                if (waitQueue.Count > 0) {
                    ...
                    obj.Close();
                    //从等待任务队列提取一个任务
                    EasyWaitItem item = waitQueue.Dequeue();
                    EasyTask nt = null;
                    ...
                    if (freeQueue.Count > 0) {
                        ...
                        nt.WorkComplete += new Action<EasyTask>(t_WorkComplete);
                    }
        ...
        public int AddTaskItem(WaitCallback TaskItem, object Context) {
            ...
            lock (lockerObj) {
                ...
                if (freeQueue.Count == 0 && len < max) {
                    ...
                    //如果线程达到max就把任务加入等待队列
                    waitQueue.Enqueue(new EasyWaitItem() { Context = Context, Works = TaskItem });
        ...
        public void CloseAfterCurrTask(int threadId) {
            //获取线程NTask对象
            EasyTask task = getNTask(threadId);
            if (task != null) {
                task.Close();
            }
        ...
        /// <param name="threadId"></param>
        /// <returns></returns>
        public EasyTask getNTask(int threadId) {
            ...
            for (int i = 0; i < workingKeys.Count; i++) {
                ...
                EasyTask n = working.ContainsKey(key) ? working[key] : null;
        ...
        public void Dispose() {
            ...
            foreach (EasyTask t in publicPool.Values) {
    ...
    public class EasyTask : IDisposable
    {
        ...
        public event Action<EasyTask> WorkComplete;  //线程完成一次操作的事件
