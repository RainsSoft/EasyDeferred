...
namespace RSG
{
    ...
    {
        object ResolveValue { get; set; }      
        Exception RejectReason { get; set; }  
        ...
        public IPromise When(ISynchronizeInvoke owerForm, Action action, bool waitTreadFinally) {
            ...
            ParameterizedThreadStart newPTS = new ParameterizedThreadStart((z) => {
                ...
                try {
                    action();
                    if (owerControl != null) {
                        InvokeIfRequired(owerControl, () => { sailor.Resolve(null); });
                    }
                    else {
                        sailor.Resolve(null);
                    }
                    ...
                    if (owerControl != null) {
                        InvokeIfRequired(owerControl, () => { sailor.Reject(exception); });
                    }
                    else {
    ...
    class SimpleGuiInvokeHelper : ISynchronizeInvoke
    {
