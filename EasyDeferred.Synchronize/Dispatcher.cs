...
namespace EasyDeferred.Synchronize
{
    ...
    ///     you must execute Dispatcher.Run() on this thread,so in console project,you must do Dispatcher.Run() in main thread
    ///     注意使用环境,当前线程必须执行Dispatcher.Run()
    /// </summary>
    public sealed class Dispatcher
    {
        ...
        private Dispatcher() {
            ...
            _queue = new ConcurrentQueue<DispatcherOperation>();//Queue();
        ...
        /// </summary>
        /// <param name="timeout">
        ///     The maximum amount of time to wait for the operation to complete.need >0
        /// </param>
        /// <param name="method">
        ///     A delegate to a method that takes parameters of the same number
        ///     and type that are contained in the args parameter.
        ...
        public object Invoke(TimeSpan timeout, DispatcherOperationCallback method, object args) {
            ...
            if (op != null) {
                System.Diagnostics.Debug.Assert(timeout.TotalMilliseconds>0);
                op.Wait(timeout);
        ...
        private ConcurrentQueue<DispatcherOperation> _queue;
    ...
    /// <summary>
    /// DispatcherObject实例应只能由调度器的线程访问。
